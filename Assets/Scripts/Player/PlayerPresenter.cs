...
public class PlayerPresenter : MonoBehaviour
{
    ...
    public void Initialize(PlayerStatus status)
    {
        ...
        // 秘宝の発動を購読、表示
        TreasureData.onAct.Subscribe(x =>
        {
        ...
        cullentEyesight = status.eyeSight;
        playerEyesightCircle.localScale = new Vector2(cullentEyesight * 2, cullentEyesight * 2);
    }

    // eyesightはプロパティでありリアクティブにできないため、仕方なくこうして監視
    private void Update()
    {
        if(_status.eyeSight != cullentEyesight)
        {
            cullentEyesight = _status.eyeSight;
            playerEyesightCircle.localScale = new Vector2(cullentEyesight * 2, cullentEyesight * 2);
        }
    }
}
