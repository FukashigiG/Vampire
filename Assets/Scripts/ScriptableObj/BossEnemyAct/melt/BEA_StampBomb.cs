...
[CreateAssetMenu(fileName = "NewBossEnemyAct", menuName = "Game Data/BossEnemyAct/StampAndBomb")]
public class BEA_StampBomb : Base_BossEnemyAct
{
    // メルトダウナー用の技の処理
    // スタンプ攻撃とボムの合わせ技
    ...
    public async override UniTask Action(EnemyCtrler_BigBoss ctrler, CancellationToken token)
    {
        ...
        {
            // ボスの位置を基準に攻撃範囲の半径付近のランダムな地点
            Vector2 target = (Vector2)ctrler.transform.position + Random.insideUnitCircle.normalized * radius * Random.Range(0.7f, 1.4f);
